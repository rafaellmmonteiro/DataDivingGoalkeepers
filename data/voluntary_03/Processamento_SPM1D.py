...
while (conts <16):                                                           #ler os arquivos dos diferentes saltos
    if (conts==0):
    ...
    while (cont <= (lmaior-lmenor-1)):
        ...
        if ((cont+lmenor) == ucsv-1):
            Vucsv1=VMM               #valor da velocidade no ultimo contato com o solo
        if (VMM>MV):
            MV=VMM
            LVM=cont
    ...
    if ((datmf[0,0]-datmf[len(datmf[:,0])-1,0]) > 0) and (dpE[0,2] > 3):    #identifica se o seujeito está em cima da plataforma e o deslocamento do eixo x da vicon para saber o lado
        if (contrae==0):
            rae = np.zeros((500,5), dtype=np.float64)
        rae[:len(VEL),contrae] = VEL
        contrae += 1
    ...
    if ((datmf[0,0]-datmf[len(datmf[:,0])-1,0]) > 0) and (dpE[0,2] < 3):   #verifica se o sujeto não está na plataforma e o deslocamento no eixo x da vicon
        if (contrne==0):
            rne = np.zeros((500,5), dtype=np.float64)
        rne[:len(VEL),contrne] = VEL
        contrne += 1
    ...
    if ((datmf[0,0]-datmf[len(datmf[:,0])-1,0]) < 0) and (dpD[0,2] > 3):
        if (contrad==0):
            rad = np.zeros((500,5), dtype=np.float64)
        rad[:len(VEL),contrad] = VEL
        contrad += 1
        
    #Regra nova a direita
    if ((datmf[0,0]-datmf[len(datmf[:,0])-1,0]) < 0) and (dpD[0,2] < 3):
        if (contrnd==0):
            rnd = np.zeros((500,5), dtype=np.float64)
        rnd[:len(VEL),contrnd] = VEL
        contrnd += 1

rae = pd.DataFrame(rae)
rne = pd.DataFrame(rne)
rad = pd.DataFrame(rad)
rnd = pd.DataFrame(rnd)

saída=pd.concat([rae,rne,rad,rnd])         #juntando todos arquivos em um só        
saída.to_csv("saída_césar_SPM.csv")                  #criando um arquivo de saída csv
