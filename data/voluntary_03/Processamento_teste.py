...
while (conts <17):                                                           #ler os arquivos dos diferentes saltos
    if (conts==0):
    ...
    if (conts==14):
        dat = pd.read_csv('trial (18).csv', sep=',', header=None, skiprows=5)
        dp = pd.read_csv("ptrial (18).csv",sep= ',', skiprows=5)
    if (conts==15):
    ...
    #zerando variáveis
    cont=maior=menor=lmaior=lmenor=DV=DH=DT=DAP=VM=VP=VMM=DVM=DHM=DAPM=Li=Vmenor=Vucsv=FPT=Lvucsv=a=LFPV=LMVAP=LFPV=LMVAPR=RdpDfn=RdpD=RdpEfn=RdpE=contcorr=sub=MV=0
    
    #lendo arquivo vicon
    datm1 = dat.to_numpy()         #convertendo para numpy
    datm = datm1[:,2:6] / 1000     #convertendo para metros e selecionando eixos x,y,z +1 para calcular escrever o deslocamento tridimensional
    ...
    sub1=np.mean(dpD[(len(dpD[:,0])-200):,0])    
    sub2=np.mean(dpD[(len(dpD[:,1])-200):,1]) 
    sub3=np.mean(dpD[(len(dpD[:,2])-200):,2])            
    while (contcorr < len(dpD[:,0])):
        dpD[contcorr,0] = dpD[contcorr,0]- sub1
        dpD[contcorr,1] = dpD[contcorr,1]- sub2
        dpD[contcorr,2] = dpD[contcorr,2]- sub3
        contcorr += 1
    ...
    if ((datmf[0,0]-datmf[len(datmf[:,0])-1,0]) > 0):   #processamento caso o salto seja a esquerda(ve se o deslocamento do eixo x foi positivo(E) ou  negativo(D))
        ...
        if (dpEf[0,2] > 1):           #identificando regra antiga (tem que ser feito isso pois o sujeito está na plataforma antes do salto e retira um pé para saltar)
        ...
        #Calculando impulso            #Está sendo feito pela área do gráfico de Força(N)xTempo(s)
        if (dpDf[0,2] > 1):           #identificando regra antiga
            RdpDfn=dpDfn[::-1]
    ...
    while (cont < 687):
        cont += 1
        DVM=float(datmf[cont-1,2])-float(datmf[cont,2])  #Deslocamento vertical do momento
        DHM=float(datmf[cont-1,0])-float(datmf[cont,0])  #Deslocamento horizontal do momento
        DAPM=float(datmf[cont-1,1])-float(datmf[cont,1]) #Deslocamento antero-posterior do momento
        DTM=(float(DHM**2)+float(DVM**2)+float(DAPM**2))**0.5      #Deslocamento tridimensional do momento
        VMM=(DTM)/0.0025                                           #Velocidade momentânea
        datm[cont-1,3]=VMM
        li +=1
        
    t5 = np.linspace(0,len(datmf[:,2])*(1/400),len(datmf[:,2]), False)
    fig1, f1_axes = plt.subplots(ncols=2, nrows=2)
    f1_axes[0,0].plot(t5,datm[:,3])
    f1_axes[0,0].plot(t5,datmf[:,3])
    #Calculando aceleração 
    a=(MV-Vmenor)/(cont11*(0.0025))
        
    #Escrevendo arquivos 
    #Regra antiga a esquerda
    if ((datmf[0,0]-datmf[len(datmf[:,0])-1,0]) > 0) and (dpE[0,2] > 3):    #identifica se o seujeito está em cima da plataforma e o deslocamento do eixo x da vicon para saber o lado
        if (contrae==0):
    ...
    if ((datmf[0,0]-datmf[len(datmf[:,0])-1,0]) > 0) and (dpE[0,2] < 3):   #verifica se o sujeto não está na plataforma e o deslocamento no eixo x da vicon
        if (contrne==0):
            Colunas = ['Deslocamento Vertical','Deslocamento Horizontal','Deslocamento Antero-Posterior','Deslocamento Tridimensional', 'velocidade média do salto',
                       'velocidade pico do salto', 'aceleração', 'força pico vertical normalizada', 'força pico horizontal normalizada', 'força pico antero-posterior normalizada',
                       'força pico tridimensional normalizada', 'impulso vertical normalizado', 'impulso horizontal normalizado', 'impulso antero-posterior normalizado', 'impulso tridimensional normalizado']
        ...
        contm=0           #contador media
        while (contm<15):        
            rne.iloc[5,contm]= np.mean([rne.iloc[0:contrne,contm]]) #calcula méia
            rne.iloc[6,contm]= np.std([rne.iloc[0:contrne,contm]]) #cálcula desvio padrão
            contm += 1

    #Regra antiga a direita
    if ((datmf[0,0]-datmf[len(datmf[:,0])-1,0]) < 0) and (dpD[0,2] > 3):
        if (contrad==0):
            Colunas = ['Deslocamento Vertical','Deslocamento Horizontal','Deslocamento Antero-Posterior','Deslocamento Tridimensional', 'velocidade média do salto',
                       'velocidade pico do salto', 'aceleração', 'força pico vertical normalizada', 'força pico horizontal normalizada', 'força pico antero-posterior normalizada',
                       'força pico tridimensional normalizada', 'impulso vertical normalizado', 'impulso horizontal normalizado', 'impulso antero-posterior normalizado', 'impulso tridimensional normalizado']
    ...
    if ((datmf[0,0]-datmf[len(datmf[:,0])-1,0]) < 0) and (dpD[0,2] < 3):
        ...
        contm=0           #contador media
        while (contm<15):        
            rnd.iloc[5,contm]= np.mean([rnd.iloc[0:contrnd,contm]])
            rnd.iloc[6,contm]= np.std([rnd.iloc[0:contrnd,contm]])
            contm += 1
saída=pd.concat([rae,rne,rad,rnd])         #juntando todos arquivos em um só        
saída.to_csv("saída_césar.txt")                  #criando um arquivo de saída csv
