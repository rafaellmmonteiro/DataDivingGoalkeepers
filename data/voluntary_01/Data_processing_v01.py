...
while (conts <15):                                                           #ler os arquivos dos diferentes saltos
    if (conts==0):
    ...
    if ((datmf[0,0]-datmf[len(datmf[:,0])-1,0]) > 0) and (dpE[0,2] > 1):    #identifica se o seujeito está em cima da plataforma e o deslocamento do eixo x da vicon para saber o lado
        if (contrae==0):
            Colunas = ['Deslocamento_Vertical','Deslocamento_Horizontal','Deslocamento_Antero-Posterior','Deslocamento_Tridimensional', 'velocidade_média',
                       'velocidade_pico', 'aceleração', 'força_pico_vertical_normalizada', 'força_pico_horizontal_normalizada', 'força_pico_antero-posterior_normalizada',
                       'força_pico_tridimensional_normalizada', 'impulso_vertical_normalizado', 'impulso_horizontal_normalizado', 'impulso_antero-posterior_normalizado', 'impulso_tridimensional_normalizado']
    ...
    if ((datmf[0,0]-datmf[len(datmf[:,0])-1,0]) > 0) and (dpE[0,2] < 1):   #verifica se o sujeto não está na plataforma e o deslocamento no eixo x da vicon
        if (contrne==0):
            Colunas = ['Deslocamento_Vertical','Deslocamento_Horizontal','Deslocamento_Antero-Posterior','Deslocamento_Tridimensional', 'velocidade_média',
                       'velocidade_pico', 'aceleração', 'força_pico_vertical_normalizada', 'força_pico_horizontal_normalizada', 'força_pico_antero-posterior_normalizada',
                       'força_pico_tridimensional_normalizada', 'impulso_vertical_normalizado', 'impulso_horizontal_normalizado', 'impulso_antero-posterior_normalizado', 'impulso_tridimensional_normalizado']
...
saída.to_csv("ouput_voluntary_01.csv")                  #criando um arquivo de saída csv
