...
while (conts <18):                                                           #ler os arquivos dos diferentes saltos
    ...
    if ((datmf[0,0]-datmf[len(datmf[:,0])-1,0]) > 0):   #processamento caso o salto seja a esquerda(ve se o deslocamento do eixo x foi positivo(E) ou  negativo(D))
        ...
        while (cont <= t2):
            ...
            FTM2=(float(dpDfn[cont,0]**2)+float(dpDfn[cont,1]**2)+float(dpDfn[cont,2]**2))**0.5  #Força tridimensional do momento
            dpDfn[cont,3]=FTM2            #escrevendo os valores da força tridimensional
            cont += 1
        ...
        if (dpD[0,2] > 1):           #identificando regra antiga (tem que ser feito isso pois o sujeito está na plataforma antes do salto e retira um pé para saltar)
            ...
            f1_axes[0,0].plot(t1, dpEfn[:,0])
            f1_axes[0,0].plot(t1, dpDfn[:,0])
            f1_axes[0,1].plot(t1, dpEfn[:,1])
            f1_axes[0,1].plot(t1, dpDfn[:,1])
            f1_axes[1,0].plot(t1, dpEfn[:,2])
            f1_axes[1,0].plot(t1, dpDfn[:,2])
            f1_axes[1,1].plot(t1, dpEfn[:,3])
            f1_axes[1,1].plot(t1, dpDfn[:,3])
            plt.title("Saltos a esquerda")
        ...
        while (cont <= t2):
            ...
            dpDfn[cont-1,3]=FTM
            FTM2=(float(dpEfn[cont,0]**2)+float(dpEfn[cont,1]**2)+float(dpEfn[cont,2]**2))**0.5  #Força tridimensional do momento
            dpEfn[cont-1,3]=FTM2            #escrevendo os valores da força tridimensional
            if (float(FTM)>float(FPT)):
                FPT= FTM                  #Valor da força pico tridimensional
...
saída.to_csv("saída_pedroalvim.csv")                  #criando um arquivo de saída csv
