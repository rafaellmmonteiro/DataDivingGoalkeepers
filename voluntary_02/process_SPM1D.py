...
while (conts <18):                                                           #ler os arquivos dos diferentes saltos
    ...
    if (conts==14):
        dat = pd.read_csv('trial (16).csv', sep=',', header=None, skiprows=5)
        dp = pd.read_csv("ptrial (16).csv",sep= ',', skiprows=5)
    if (conts==15):
...
#interpolando dados rae       

vrae1=vrae2=vrae3=vrae4=0

vrae1=rae[:rae[:,0].argmin(), 0]
vrae2=rae[:rae[:,1].argmin(), 1]
vrae3=rae[:rae[:,2].argmin(), 2]
vrae4=rae[:rae[:,3].argmin(), 3]

fig1, f1_axes = plt.subplots(ncols=2, nrows=2)

V1= [vrae1,vrae2,vrae3,vrae4]
V1= spm1d.util.interp(V1, Q=101)

VX1=V1.T
f1_axes[0,0].plot(VX1, 'r')

#interpolando dados rne

vrne1=vrne2=vrne3=vrne4=vrne5=0

vrne1=rne[:rne[:,0].argmin(), 0]
vrne2=rne[:rne[:,1].argmin(), 1]
vrne3=rne[:rne[:,2].argmin(), 2]
vrne4=rne[:rne[:,3].argmin(), 3]
vrne5=rne[:rne[:,4].argmin(), 4]

V2= [vrne1,vrne2,vrne3,vrne4]
V2= spm1d.util.interp(V2, Q=101)

VX2=V2.T
f1_axes[0,0].plot(VX2, 'b')

#interpolando dados rad       

vrad1=vrad2=vrad3=vrad4=0

vrad1=rad[:rad[:,0].argmin(), 0]
vrad2=rad[:rad[:,1].argmin(), 1]
vrad3=rad[:rad[:,2].argmin(), 2]
vrad4=rad[:rad[:,3].argmin(), 3]

V3= [vrad1,vrad2,vrad3,vrad4]
V3= spm1d.util.interp(V3, Q=101)

VX3=V3.T
f1_axes[0,0].plot(VX3, 'g')

#interpolando dados rnd

vrnd1=vrnd2=vrnd3=vrnd4=0

vrnd1=rnd[:rnd[:,0].argmin(), 0]
vrnd2=rnd[:rnd[:,1].argmin(), 1]
vrnd3=rnd[:rnd[:,2].argmin(), 2]
vrnd4=rnd[:rnd[:,3].argmin(), 3]

V4= [vrnd1,vrnd2,vrnd3,vrnd4]
V4= spm1d.util.interp(V4, Q=101)

VX4=V4.T
f1_axes[0,0].plot(VX4, 'y')

#realizando o teste SPM
#######################################################
t  = spm1d.stats.ttest_paired(V1, V3)
...
saída.to_csv("saída_bruno_SPM.csv")                  #criando um arquivo de saída csv
