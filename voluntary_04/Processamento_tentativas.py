...
while (conts <19):                                                           #ler os arquivos dos diferentes saltos
    if (conts==0):
    ...
    if ((datmf[0,0]-datmf[len(datmf[:,0])-1,0]) < 0) and (dpD[0,2] > 1):
        ...
        contm=0           #contador media
        while (contm<15):        
            rad.iloc[5,contm]= np.mean([rad.iloc[0:5,contm]])
            rad.iloc[6,contm]= np.std([rad.iloc[0:5,contm]])
            contm += 1
...
saída.to_csv("saída_gabrielbuzzetto.csv")                  #criando um arquivo de saída csv
